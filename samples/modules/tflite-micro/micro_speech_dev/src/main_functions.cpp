...
void main(void) {
  printk("Starting TensorFlow Lite Micro Speech example\n");
  ...
  model = tflite::GetModel(g_model);
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    printk("Model provided is schema version %d not equal to supported version %d.\n",
           model->version(), TFLITE_SCHEMA_VERSION);
    return;
  }
  ...
  static tflite::MicroMutableOpResolver<4> micro_op_resolver;
  if (micro_op_resolver.AddDepthwiseConv2D() != kTfLiteOk) {
    printk("Failed to add DepthwiseConv2D op\n");
    return;
  }
  if (micro_op_resolver.AddFullyConnected() != kTfLiteOk) {
    printk("Failed to add FullyConnected op\n");
    return;
  }
  if (micro_op_resolver.AddSoftmax() != kTfLiteOk) {
    printk("Failed to add Softmax op\n");
    return;
  }
  if (micro_op_resolver.AddReshape() != kTfLiteOk) {
    printk("Failed to add Reshape op\n");
    return;
  }
  ...
  TfLiteStatus init_status = InitAudioRecording();
  if (init_status != kTfLiteOk) {
    printk("Unable to initialize audio\n");
    return;
  }
  ...
  while (1) {
    ...
    TfLiteStatus feature_status = feature_provider->PopulateFeatureData(
        previous_time, current_time, &how_many_new_slices);
    if (feature_status != kTfLiteOk) {
      printk("Feature generation failed\n");
      return;
    }
    previous_time += how_many_new_slices * kFeatureSliceStrideMs;
    // If no new audio samples have been received since last time, don't bother
    ...
    TfLiteStatus process_status = recognizer->ProcessLatestResults(
        output, current_time, &found_command, &score, &is_new_command);
    if (process_status != kTfLiteOk) {
      printk("RecognizeCommands::ProcessLatestResults() failed\n");
      return;
    }
